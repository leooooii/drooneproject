...
function FourPointForm(q0, q1, q2, q3) {
    ...
    // add four points
    var point, tangent;
    point = new THREE.Mesh(new THREE.CircleGeometry(0.1, 12), new THREE.MeshBasicMaterial());
    point.position.copy(q0);

    curveGroup.add(point);
    tangent = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.5), new THREE.MeshBasicMaterial());
...
function initThree() {
 ...
 // airplane.scale.set(3,3,3);
	...
	airplane.add(mesh);*/
     curve = FourPointForm(new THREE.Vector3(0, 5, 0), // q0
    new THREE.Vector3(Math.sin(Math.PI/6)*5, 5, Math.cos(Math.PI/6)*5-5), // q1
    new THREE.Vector3(Math.sin(Math.PI/3)*5, 5, Math.cos(Math.PI/3)*5-5), // q2
    new THREE.Vector3(5, 5, -5)); // q3
    scene.add(curve);
