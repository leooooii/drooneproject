...
function buildScene() {
 ...
 // scene.background = cubeMap;
  ...
  var plane2= new THREE.Mesh(new THREE.PlaneGeometry(4,4), new THREE.MeshBasicMaterial({color:0xff0000,side:THREE.DoubleSide,  transparent: true,
  opacity: 0.2,
  visible: true}));
  ...
  showplane1.position.x=3;

   showplane2 =new THREE.Object3D();
  var plane3= new THREE.Mesh(new THREE.PlaneGeometry(4,4), new THREE.MeshBasicMaterial({color:0xff0000,side:THREE.DoubleSide,  transparent: true,
  opacity: 0.2,
  visible: true}));
...
var angle1 = angle2 = 0;
var Raidus=2;
var  point1, curve2,curveObjectt, geometryCurve, materialCurve,curveCcw,curveCw;
   ...
   var params1cw = {P0x: Raidus, P0y: 0 ,P0z:0,
            P1x: 0.5*Raidus*Math.sqrt(2)+Raidus-Raidus, P1y: 0 ,P1z:Raidus*0.5*Math.sqrt(2),
            P2x: Raidus-Raidus, P2y: 0,P2z:Raidus,
            P3x: -0.5*Raidus*Math.sqrt(2)+Raidus-Raidus, P3y: 0,P3z:Raidus*0.5*Math.sqrt(2),
            P4x: -Raidus, P4y: 0,P4z:0,
            P5x: -0.5*Raidus*Math.sqrt(2)+Raidus-Raidus, P5y: 0 ,P5z:-Raidus*0.5*Math.sqrt(2),
            P6x: Raidus-Raidus, P6y: 0,P6z:-Raidus,
            P7x: 0.5*Raidus*Math.sqrt(2)+Raidus-Raidus, P7y: 0,P7z:-Raidus*0.5*Math.sqrt(2),      
            steps: 50,close:true};
...
createCatmullRomCurve3 = function (cpList, steps,closes,angle,plane) {
  ...
  curveCcw.points[0].x+=3
  curveCcw.points[1].x+=3
  curveCcw.points[2].x+=3
  curveCcw.points[3].x+=3
  curveCcw.points[4].x+=3
  curveCcw.points[5].x+=3
  curveCcw.points[6].x+=3
  curveCcw.points[7].x+=3
...
createCwloop= function (cpList, steps,closes,angle,plane) {

  var N = Math.round(steps)+1 || 20;
  var geometry = new THREE.Geometry();
  curveCw = new THREE.CatmullRomCurve3();
  var cp = cpList[0];
  curveCw.points[0] = new THREE.Vector3(cp[0], cp[1], cp[2]);
  cp = cpList[1];
  curveCw.points[1]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
  cp = cpList[2];
  curveCw.points[2]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
  
  var rotatePoint  =  new THREE.Vector3(0, 0,1);
  rotatePoint=rotatePoint.clone().normalize();

  cp = cpList[3];
  curveCw.points[3]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
  cp = cpList[4];
  curveCw.points[4]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
  cp = cpList[5];
  curveCw.points[5]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
    cp = cpList[6];
  curveCw.points[6]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
    cp = cpList[7];
  curveCw.points[7]  = new THREE.Vector3(cp[0], cp[1], cp[2]);
  curveCw.points[0].applyAxisAngle(rotatePoint,angle);
  curveCw.points[1].applyAxisAngle(rotatePoint,angle);
  curveCw.points[2].applyAxisAngle(rotatePoint,angle);
  curveCw.points[3].applyAxisAngle(rotatePoint,angle);
  curveCw.points[4].applyAxisAngle(rotatePoint,angle);
  curveCw.points[5].applyAxisAngle(rotatePoint,angle);
  curveCw.points[6].applyAxisAngle(rotatePoint,angle);
  curveCw.points[7].applyAxisAngle(rotatePoint,angle);
  curveCw.points[0].y+=5
  curveCw.points[1].y+=5
  curveCw.points[2].y+=5
  curveCw.points[3].y+=5
  curveCw.points[4].y+=5
  curveCw.points[5].y+=5
  curveCw.points[6].y+=5
  curveCw.points[7].y+=5
  curveCw.points[0].x-=3
  curveCw.points[1].x-=3
  curveCw.points[2].x-=3
  curveCw.points[3].x-=3
  curveCw.points[4].x-=3
  curveCw.points[5].x-=3
  curveCw.points[6].x-=3
  curveCw.points[7].x-=3
  curveCw.closed=closes;
  
  var j, stepSize = 1/(N-1);
  for (j = 0; j < N; j++) {
      geometry.vertices.push( curveCw.getPoint(j * stepSize) );
  }
  return geometry;
};
 ...
 function CreateCwCurve(angle){
    //scene.remove(curve1);
    var controlPoints1 = [
      [params1cw.P0x, params1cw.P0y, params1cw.P0z],
      [params1cw.P1x, params1cw.P1y, params1cw.P1z],
      [params1cw.P2x, params1cw.P2y, params1cw.P2z],
      [params1cw.P3x, params1cw.P3y, params1cw.P3z],
      [params1cw.P4x, params1cw.P4y, params1cw.P4z],
      [params1cw.P5x, params1cw.P5y, params1cw.P5z],
      [params1cw.P6x, params1cw.P6y, params1cw.P6z],
      [params1cw.P7x, params1cw.P7y, params1cw.P7z]
      ];
 var curveGeom1 = createCwloop(controlPoints1, params1cw.steps,params1cw.close,angle);
  
  var mat = new THREE.LineBasicMaterial( { color: 0xffffff, linewidth: 30 } );
  curve2cw = new THREE.Line( curveGeom1, mat );
  //scene.add(curve1);
};
  ...
  var curveGeom1 = createCatmullRomSCurve3(controlPoints1, params2.steps,params2.close,angle);
