...
function yawServo2 (body, dt,yawReff,yawAnglee) {  // pass dt for integral control
	let error = yawReff - yawAnglee;	
	
	if (yawServo2.integral === undefined)
		yawServo2.integral = 0;
	yawServo2.integral += error*dt;
	
	var diff = 0;
	if (yawServo2.lastError !== undefined) {
	   diff = (error - yawServo2.lastError)/dt;
//	   yawServo.lastError = error;
	}  
	yawServo2.lastError = error;
	
	const KP = .3;
	const KI = 0//2;  // PD works, but not PID ...
	const KD = 15;
	let r = KP*error + KI*yawServo2.integral + KD*diff;
	return r;
}
...
function yawServo3 (body, dt,yawReff,yawAnglee) {  // pass dt for integral control
	...
	if (yawServo3.integral === undefined)
		yawServo3.integral = 0;
	yawServo3.integral += error*dt;
	
	var diff = 0;
	if (yawServo3.lastError !== undefined) {
	   diff = (error - yawServo3.lastError)/dt;
//	   yawServo.lastError = error;
	}  
	yawServo3.lastError = error;
