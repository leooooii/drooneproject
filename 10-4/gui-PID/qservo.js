...
var RollP,RollI,RollD,PitchP,PitchI,PitchD;
...
function rollServo (body, dt) {  // pass dt for integral control
	...
	RollP= gcontrols.rollP;
	RollI = gcontrols.rollI;//2;  // PD works, but not PID ...
	RollD = gcontrols.rollD;;
	let r = RollP*error + RollI*rollServo.integral + RollD*diff;
	//console.log ('r:'+r)
	return r;
}
...
function pitchServo (body, dt) {
  ...
  PitchP=gcontrols.pitchP;
  PitchI=gcontrols.pitchI;
  PitchD=gcontrols.pitchD;
	let r = PitchP*error + PitchI*pitchServo.integral + PitchD*diff;
	//console.log ('r:'+r)
	return r;
}
